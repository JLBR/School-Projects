...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    while(compare(getDynArr(heap, currentPos/2), val)==1){
	   swapDynArr(heap, currentPos, currentPos/2);
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    if (rightChild < max) { /* we have two children */
	   int smallestChild = _smallerIndexHeap(heap, leftChild, rightChild);
        smallestChild = _smallerIndexHeap(heap, pos, smallestChild);
...
void removeMinHeap(DynArr *heap)
{
      ...
      putDynArr(heap, 0, getDynArr(heap, last)); /* Copy the last element to the first  position */
