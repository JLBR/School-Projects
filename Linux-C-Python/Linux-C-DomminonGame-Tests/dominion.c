...
/*
Adventure Card: “Reveal cards from your deck until you reveal 2 Treasure cards.
...
int playAdventurer(int currentPlayer, struct gameState *state)
{
	...
	int tempNumCards = 0;// this is the counter for the temp hand

	while(drawnTreasure < ADVENTURE_TREASURE_DRAW)//draw 2 treasure cards
	{
		...
		{
			temphand[tempNumCards]=cardDrawn;
...
Additional Rules: The other players must draw a card whether they want to or not. All players should shuffle as necessary.
...
int playCouncilRoom(int currentPlayer, struct gameState *state, int handPos)
{
	...
	discardCard(handPos, currentPlayer, state, DISCARD_TO_PLAYED);
...
/*
Mine: "	Trash a Treasure card from your hand. Gain a Treasure card costing up to 3 Coins more; put it into your hand. "

Additional Rules: Generally, you can trash a Copper card and gain a Silver, or trash a Silver card and gain a Gold.
However, you could also trash a Treasure to gain the same Treasure or a cheaper one.
The gained card goes in your hand; thus, you can spend it the same turn.
If you don't have a Treasure card in your hand to trash, you can’t gain anything.

This function does not account for new cards being added at a later date.
*/
int playMine(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2)
{
	...
	trashedCard = state->hand[currentPlayer][choice1];

	//check that choice 1 is copper silver or gold
	if ( (state->hand[currentPlayer][choice1] < copper)
		|| (state->hand[currentPlayer][choice1] > gold) )
		return EFFECT_FAIL;

	//check that choice 2 is a card
	if ( (choice2 > treasure_map)
		|| (choice2 < curse) )
		return EFFECT_FAIL;

	//check that choice 2 is cost less than 3+ the value of the card being upgraded
	if ( (getCost(state->hand[currentPlayer][choice1]) + MINE_UPGRADE_COINS)
		> getCost(choice2) )
		return EFFECT_FAIL;

	gainCard(choice2, state, TO_HAND, currentPlayer);
	...
	for (i = 0; i < state->handCount[currentPlayer]; i++)
	{
	    ...
	    {
			discardCard(i, currentPlayer, state, DISCARD_TO_PLAYED);
			break;
	    }
	}
...
/*
Remodel: Trash a card from your hand. Gain a card costing up to 2 Coins more than the trashed card.

Additional Rules: You cannot trash the Remodel as it isn't in your hand when you resolve it
(you can trash a different Remodel card from your hand). If you do not have a card to trash,
you cannot gain a card from the Remodel. The gained card goes in your Discard pile.
You can only gain cards from the Supply. The gained card need not cost exactly 2 coins more than the trashed card;
it can cost that much or any amount less. You cannot use coins from Treasures or previous Actions (like the Market)
to increase the cost of the card you gain. You can trash a card to gain a copy of the same card.
...
int playRemodel(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2)
{
	...
	if ( (getCost(state->hand[currentPlayer][choice1]) + REMODEL_BONUS_COINS)
		> getCost(choice2) )
	{
		return EFFECT_FAIL;
	}

	gainCard(choice2, state, TO_DISCARD, currentPlayer);
...
//  int cardDrawn;
  ...
  switch( card )
    ...
    case remodel:
        return playRemodel(currentPlayer, state, handPos, choice1, choice2);
